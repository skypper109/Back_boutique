                ...
                <tr class="{{ !$user->is_active ? 'deactivated-row' : '' }}">
                    ...
                    <td style="text-align: right;">
                        <div class="action-buttons">
                            <button onclick="openPasswordModal('{{ $user->id }}', '{{ $user->name }}')" class="action-btn" title="Réinitialiser MDP">
                            ...
                            <form action="{{ route('admin.users.toggle-status', $user->id) }}" method="POST" style="display: inline;">
                                @csrf
                                <button type="submit" class="action-btn" title="{{ $user->is_active ? 'Désactiver' : 'Activer' }}">
                                    <i class="bi {{ $user->is_active ? 'bi-person-dash text-danger' : 'bi-person-check text-success' }}"></i>
                                </button>
                            </form>

                            <form action="{{ route('admin.users.destroy', $user->id) }}" method="POST" style="display: inline;" onsubmit="return confirm('ALERTE: Suppression définitive du compte. Continuer ?');">
                                @csrf
                                @method('DELETE')
...
<div id="createUserModal" class="modal">
    <div class="modal-content">
        <div class="card-title">
            ...
            <button onclick="document.getElementById('createUserModal').style.display='none'" class="text-muted" style="background: none; border: none; cursor: pointer;">
        ...
        <form action="{{ route('admin.boutiques.users.store', $boutique->id) }}" method="POST">
            ...
            <div class="grid-2" style="margin-top: 32px;">
                <button type="button" onclick="document.getElementById('createUserModal').style.display='none'" class="btn" style="background: #f1f5f9; color: var(--text-main);">
                    Annuler
                </button>
...
<div id="passwordModal" class="modal">
    <div class="modal-content" style="border-top: 5px solid var(--accent);">
        <div class="card-title">
            ...
            <button onclick="document.getElementById('passwordModal').style.display='none'" class="text-muted" style="background: none; border: none; cursor: pointer;">
                <i class="bi bi-x-lg"></i>
            </button>
        </div>
        ...
        <form id="passwordForm" method="POST">
            ...
            <div class="grid-2" style="margin-top: 32px;">
                <button type="button" onclick="document.getElementById('passwordModal').style.display='none'" class="btn" style="background: #f1f5f9; color: var(--text-main);">
                    Annuler
                </button>
